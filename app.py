# ═════════════════════════════════════
# ChatTesis PRO — MULTIAGENTE + OUTPUT INTELIGENTE
# ═════════════════════════════════════
...
def login():
    st.sidebar.title("🔐 Acceso a Chat Acredita (EISC)")
...
Instrucciones:
- Si la información es conceptual → genera un resumen en viñetas
- Si es comparativa → usa tabla Markdown
- Combina si aplica
...
# 🔥 CONTADOR (ANTES DE SESSION PRINCIPAL)
...
function forceScroll() {
    ...
    const el = parent.getElementById("bottom");
    if (el) {
        el.scrollIntoView({ behavior: "smooth" });
    }
}

// Ejecutar múltiples veces (clave en Streamlit)
...
st.sidebar.metric("Visitas", st.session_state.visits)

with st.sidebar.expander("🧠 ¿Qué hace el chatbot?", expanded=True):
    st.markdown("""
### 🔍 Capacidades

- 📚 Búsqueda en documentos académicos  
...
st.markdown("""
<div class="footer">
Universidad del Valle • Grupo GUIA • ChatAcredita PRO
</div>
""", unsafe_allow_html=True)