...
# ─────────────────────────────────────────────
# DEEPSEEK 14B — fine-tuneado acreditación CNA (raulgdp/deepseek14b-acredita)
# ─────────────────────────────────────────────
...
def generate_deepseek_response(
    ...
    try:
        ...
        import traceback
        print(f"🔥 ERROR DeepSeek: {traceback.format_exc()}")
...
def hybrid_search_rrf(query: str, query_variants: list[str], use_feedback: bool = False, k_rrf: int = 60) -> list[dict]:
    ...
    for pid, score in sorted_ids:
        ...
        if payload.get("text"):
            results.append({"id": pid, "text": payload["text"], "source": payload.get("source", "desconocido"), "rrf_score": round(score, 4)})
    return results

def rerank_results(query: str, results: list[dict]) -> list[dict]:
    """Reranking con BGE-Reranker-v2-m3 sobre los candidatos del RRF híbrido."""
    if not results:
        return []
    ...
    try:
        ...
        for r, score in zip(results, scores):
            ...
            r["final_score"] = 0.7 * r["rerank_score"] + 0.3 * r.get("rrf_score", 0.0)
    except Exception as e:
        st.warning(f"⚠️ Error en reranker, usando solo RRF: {str(e)[:120]}")
        for r in results:
    ...
    if st.session_state.use_deepseek:
        ...
        with st.spinner("Cargando DeepSeek 14B acreditación CNA..."):
            try:
                ...
                st.error(f"❌ Error cargando DeepSeek: {str(e)[:100]}")
